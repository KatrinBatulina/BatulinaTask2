...
public class Matrix {
    ...
    public void MatrixVoid(int rows, int cols, int randomLimit) {
        this.rows = rows;
        this.cols = cols;
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                element[i][j] = (int) Math.round(Math.abs(randomLimit) * Math.random() + 1);
            }
        }
    }
